...
export const translationSchema = z.object({ title: text, summary: z.array(text).min(3).max(5), facts: z.array(factSchema).max(100), news: z.array(newsSchema).max(5), events: z.array(eventSchema).max(50), macro: z.array(macroSchema).max(20), scenarios: z.array(scenarioSchema).max(3), sourcesAndLimitations: text, levels: z.array(levelSchema).max(100).default([]) }).strict()
...
export function emptyTranslation(): Translation { return { title:'',summary:['','',''],facts:[],news:[],macro:[],scenarios:[],levels:[],events:[],sourcesAndLimitations:'' } }
