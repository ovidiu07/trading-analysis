...
| Today | Shared event display with source links, units, source timezone, retrieval/publication times and unavailable forecasts | Uses the preparation's captured briefing when present; imports never replace published or Ready/history content |
...
6. Published revisions, source revisions, identity links and stored preparation compositions remain immutable. Today reads the preparation capture instead of independently selecting a later publication. A read-time defensive copy withholds premature/unknown actuals and unsourced forecast/previous values in older payloads without mutating stored history.
