...
;(async () => {
  ...
  await context.addInitScript(() => { localStorage.setItem('token', 'synthetic-browser-fixture'); localStorage.setItem('app.language', 'en'); localStorage.setItem('app.themePreference', 'dark') })
  ...
  for (const width of [320,360,390,430,768,1024,1280,1440,1920]) {
    ...
    if ([390,1440].includes(width)) await page.screenshot({ path: path.join(__dirname, `today-prepare-${width}.png`), fullPage: true })
  }
  await page.setViewportSize({ width: 390, height: 844 })
  await page.getByLabel('One session focus', { exact: true }).fill('Wait for confirmation')
  await page.reload()
  await page.getByDisplayValue?.('Wait for confirmation')
  if (await page.getByLabel('One session focus', { exact: true }).inputValue() !== 'Wait for confirmation') throw new Error('Draft recovery failed')
  await page.getByRole('button', { name: 'Start session', exact: true }).click()
  await page.getByText('Execution timeline', { exact: true }).waitFor()
  await page.getByRole('button', { name: 'Review', exact: true }).click()
  const tradeButton = page.getByRole('button').filter({ hasText: 'DEMO' }).first()
  await tradeButton.click()
  await page.getByRole('dialog').waitFor()
  await page.getByRole('button', { name: 'Followed my plan', exact: true }).click()
  await page.getByLabel('Short review note').fill('Confirmation recorded before entry')
  ...
  await page.getByLabel('What carries forward?').click()
  await page.getByRole('option', { name: 'Something to repeat', exact: true }).click()
  await page.getByLabel('One next-session focus').fill('Keep waiting for confirmation')
  await page.getByRole('button', { name: 'Finish review', exact: true }).click()
  await page.getByText('Review completed and saved.', { exact: false }).waitFor()
  ...
  await page.screenshot({ path: path.join(__dirname, 'today-review-complete-390.png'), fullPage: true })
  await page.getByRole('button', { name: 'Reopen review', exact: true }).click()
  await page.getByText('Review your decisions', { exact: true }).waitFor()
  results.push({ completion: true, reopening: saved.data.state === 'REVIEW', savedRevision: saved.revision })
  fs.writeFileSync(path.join(__dirname, 'browser-results.json'), JSON.stringify({ fixtureOnly: true, results, unexpected, errors }, null, 2))
  await browser.close()
