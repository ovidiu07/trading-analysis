...
class BriefingValidatorTest {
 ...
 @Test void editorialImpactIsOptionalAndBackwardCompatible() throws Exception {var d=document();var events=(com.fasterxml.jackson.databind.node.ArrayNode)d.path("translations").path("en").path("events");events.addObject().put("id","event-1").put("scheduledAt","2026-09-07T12:30:00Z").put("timezone","America/New_York").put("region","US").put("name","QA").put("source","QA").put("status","UPCOMING");var event=validator.parse(d,true,now).translations().get("en").events().getFirst();assertThat(event.impact()).isNull();var eventNode=(com.fasterxml.jackson.databind.node.ObjectNode)events.get(0);eventNode.put("impact","HIGH");assertThat(validator.parse(d,true,now).translations().get("en").events().getFirst().impact()).isEqualTo(BriefingDocument.EventImpact.HIGH);eventNode.put("impact","AUTOMATIC");assertThatThrownBy(()->validator.parse(d,true,now)).hasMessageContaining("Invalid briefing schema");}
 @Test void acceptsSourceAttributedInstrumentScopedEditorialLevel() throws Exception {var d=document();var levels=(com.fasterxml.jackson.databind.node.ArrayNode)((com.fasterxml.jackson.databind.node.ObjectNode)d.path("translations").path("en")).putArray("levels");levels.addObject().put("id","ger40-bsl").put("instrument","GER40").put("label","BSL").put("value",19000).put("unit","index points").put("source","Admin-reviewed briefing").put("sourceUrl","https://example.org/source").put("rationale","Level supplied by the editor");assertThat(validator.parse(d,true,now).translations().get("en").levels()).hasSize(1);}
 @Test void rejectsUnsupportedInstrumentOrNonPositiveEditorialLevel() throws Exception {var d=document();var levels=(com.fasterxml.jackson.databind.node.ArrayNode)((com.fasterxml.jackson.databind.node.ObjectNode)d.path("translations").path("en")).putArray("levels");levels.addObject().put("id","bad-level").put("instrument","SPX500_USD").put("label","SUPPORT").put("value",0).put("unit","points").put("source","QA").put("rationale","Must be rejected");assertThatThrownBy(()->validator.parse(d,true,now)).hasMessageContaining("must match");}
 @Test void rejectsDuplicateEditorialLevelIds() throws Exception {var d=document();var levels=(com.fasterxml.jackson.databind.node.ArrayNode)((com.fasterxml.jackson.databind.node.ObjectNode)d.path("translations").path("en")).putArray("levels");for(int i=0;i<2;i++)levels.addObject().put("id","same").put("instrument","GER40").put("label","SUPPORT").put("value",18000+i).put("unit","points").put("source","QA").put("rationale","Published context");assertThatThrownBy(()->validator.parse(d,true,now)).hasMessageContaining("duplicate id same");}
